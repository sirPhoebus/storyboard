...
app.delete('/api/projects/:id', (req: any, res: any) => {
    ...
    try {
        ...
        const projectDir = path.join(uploadsDir, projectId);
        if (fs.existsSync(projectDir)) {
            console.log(`??? Deleting project folder: ${projectDir}`);
            ...
            if (content.url) {
                ...
                if (countResult.count === 0) {
                    if (fs.existsSync(filePath)) {
                        console.log(`??? Hard deleting file: ${filePath}`);
                        fs.unlinkSync(filePath);
                    }
                } else {
                    console.log(`??? Preserving file ${fileName}, used by ${countResult.count} other elements`);
                }
            }
        });
    } catch (err) {
            ...
            try {
                ...
                if (content.url) {
                    ...
                    if (filePath && fs.existsSync(filePath)) {
                        ...
                        console.warn(`?? [Download] File missing for element ${el.id}: ${filePath} (URL: ${content.url})`);
                        missedCount++;
